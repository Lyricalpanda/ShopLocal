...
- (void)viewDidLoad
{
    ...
    MKNetworkOperation *_op = [sharedEngine getBusineses];
    __weak MKNetworkOperation *op=_op;
    [op addCompletionHandler:^(MKNetworkOperation *completedOperation)
     {
         [self parseBusinesses:[[op responseString] JSONValue]];
...
- (void)plotBusinesses
{
    ...
    for (int i =0; i < [filteredBusinesses count]; i++)
    {
        ...
        NSUInteger result = [businessMapView.annotations indexOfObjectPassingTest:^BOOL(id obj, NSUInteger idx, BOOL *stop) {
            if ([obj isKindOfClass:[BusinessAnnotation class]]) {
                BusinessAnnotation *ba = (SLBusinessAnnotation*) obj;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.businessImage setUrl:[NSURL URLWithString:currentBusiness.url]];
